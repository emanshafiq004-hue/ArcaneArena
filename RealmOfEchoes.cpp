...
// Phantom survival: 30 s total, last 0.2 s is the "final sting" window
static const float kSurvivalSeconds = 30.f;
static const float kFinalStingThreshold = 0.2f;   // seconds before timer ends
...
void RealmOfEchoes::setupAudio()
{
    ...
    audio.preloadOptional("ui_move", "assets/audio/ui_move.ogg");
    audio.preloadOptional("Click", "assets/audio/Main_Confirm.ogg");
    // Battle SFX (kept as sounds — short clips)
    audio.preloadOptional("attack", "assets/audio/attack.mp3");
    audio.preloadOptional("hit", "assets/audio/battle_hit.ogg");
    audio.preloadOptional("block", "assets/audio/battle_block.ogg");
    audio.preloadOptional("ability", "assets/audio/ability.ogg");
    audio.preloadOptional("heal", "assets/audio/heal.ogg");
    // Boss defeat one-shot SFX
    audio.preloadOptional("boss_defeat", "assets/audio/boss_defeat.ogg");
    // Guidance / negotiation SFX
    audio.preloadOptional("guidance", "assets/audio/Guidance_spirit.ogg");
}
...
void RealmOfEchoes::loadRegionBackground(int regionIndex)
{
    ...
    loadStaticBackground({ texPaths[regionIndex][0], texPaths[regionIndex][1] });
    // Stop any currently playing music before starting the region track
    audio.Stop();
    audio.PlayMusic(musicPaths[regionIndex], 50.f);
}
...
string RealmOfEchoes::showNameEntryScreen()
{
    ...
    Text nameDisplay(font);
    nameDisplay.setCharacterSize(28);
    // Visible warm brown matching parchment theme
    nameDisplay.setFillColor(Color(80, 50, 20));
    nameDisplay.setOutlineColor(Color(220, 180, 100, 120));
    nameDisplay.setOutlineThickness(1.f);
    nameDisplay.setStyle(Text::Bold);
    const float midY = static_cast<float>(gw.getHeight()) * 0.55f;
    ...
    while (gw.isOpen())
    {
        ...
        gw.clear();
        drawBackdrop();
        // Darker rich brown for parchment — readable
        drawCenteredText("ENTER YOUR NAME, VEILWALKER",
            static_cast<float>(gw.getHeight()) * 0.28f, 26, Color(80, 50, 20));
        drawCenteredText("(PRESS ENTER TO CONFIRM)",
            static_cast<float>(gw.getHeight()) * 0.93f, 20, Color(180, 130, 60));
        gw.getWindow().draw(nameDisplay);
        gw.display();
    }
    return "WANDERER";
...
{
    sf::Font storyFont;
    if (!AssetLoader::openFontWithFallback(storyFont, { "assets/font/StoryUncialAntiqua.ttf", "assets/font/Philosopher-Bold.ttf" }))
        throw runtime_error("RealmOfEchoes: Cannot load font!");
    loadStaticBackground({
        "assets/texture/bg_roe_story_parchment.png",
        "assets/texture/default.png"
    ...
    while (gw.isOpen() && !done)
    {
        ...
        while (getline(ss, line))
        {
            Text lineText(storyFont);
            lineText.setString(upperText(line));
            lineText.setCharacterSize(16);
            // Dark brown on parchment — good contrast
            lineText.setFillColor(Color(70, 40, 15));
            lineText.setPosition({ 50.f, y });
            lineText.setStyle(Text::Bold);
            gw.getWindow().draw(lineText);
            y += 26.f;
...
void RealmOfEchoes::showWorldMapScreen(int& selectedRegion)
{
    ...
    if (player && player->getHp() <= 0)
    {
        ...
        gw.clear(); drawBackdrop();
        drawCenteredText("REVIVED! RETURNING TO PREVIOUS REGION...",
            static_cast<float>(gw.getHeight()) * 0.45f, 20, Color(0, 230, 255));
        drawCenteredText("PRESS ENTER TO CONTINUE",
            static_cast<float>(gw.getHeight()) * 0.60f, 16, Color(200, 80, 255));
        gw.display();
        sf::sleep(sf::seconds(2.f));
    ...
    cursor.setPoint(1, { 0.f,  14.f });
    cursor.setPoint(2, { 12.f, 7.f });
    cursor.setFillColor(Color(0, 230, 255));
    ...
    while (gw.isOpen() && !quitToHub)
    {
        ...
        gw.clear();
        drawBackdrop();
        // World map text colors — warm gold on the map image
        drawCenteredText("THE VEILLANDS", 40.f, 30, Color(255, 220, 80));
        drawCenteredText("UP/DOWN  ENTER  TO SELECT", 72.f, 14, Color(200, 170, 90));

        for (size_t i = 0; i < optionTexts.size(); i++)
        {
            bool isSel = (static_cast<int>(i) == selected);
            optionTexts[i].setFillColor(isSel ? Color(255, 240, 80) : Color(200, 170, 90));
            optionTexts[i].setCharacterSize(isSel ? 24 : 22);
            FloatRect b = optionTexts[i].getLocalBounds();
            optionTexts[i].setOrigin({ b.size.x / 2.f, b.size.y / 2.f });
            const float optY = 120.f + static_cast<float>(i) * 48.f;
        ...
        if (player)
        {
            ...
            stats.setString(upperText(
                ...
                "  COINS " + to_string(player->getCoins())));
            stats.setCharacterSize(14);
            stats.setFillColor(Color(100, 255, 120));
            stats.setOutlineColor(Color(0, 0, 0, 160));
...
void RealmOfEchoes::runNegotiationScreen(const string& msg)
{
    ...
    if (AssetLoader::loadTextureWithFallback(guidanceTex, { "assets/characters/guidance_spirit.png" })) {
        ...
        const float sc = targetH / static_cast<float>(guidanceTex.getSize().y);
        guidanceSpr->setScale({ sc, sc });
        guidanceSpr->setPosition({ static_cast<float>(gw.getWidth()) * 0.75f,
                                   static_cast<float>(gw.getHeight()) * 0.35f });
    }

    // Load Veilwalker VNegotiation sprite (left side)
    Texture vNegTex;
    optional<Sprite> vNegSpr;
    if (AssetLoader::loadTextureWithFallback(vNegTex, { "assets/characters/VNegotiation.png" })) {
        vNegTex.setSmooth(true);
        vNegSpr.emplace(vNegTex);
        const float targetH = static_cast<float>(gw.getHeight()) * 0.35f;
        const float sc = targetH / static_cast<float>(vNegTex.getSize().y);
        vNegSpr->setScale({ sc, sc });
        vNegSpr->setPosition({ static_cast<float>(gw.getWidth()) * 0.08f,
                               static_cast<float>(gw.getHeight()) * 0.35f });
    }
    ...
    while (gw.isOpen() && !done)
    {
        ...
        gw.clear();
        drawBackdrop();
        // Purple accent title on dark backdrop
        drawCenteredText("GUIDANCE",
            static_cast<float>(gw.getHeight()) * 0.2f, 26, Color(210, 100, 255));
        if (vNegSpr) gw.getWindow().draw(*vNegSpr);
        if (guidanceSpr) gw.getWindow().draw(*guidanceSpr);
        Text body(font);
        body.setString(upperText(msg));
        body.setCharacterSize(15);
        body.setFillColor(Color(230, 220, 255));
        body.setPosition({ 40.f, static_cast<float>(gw.getHeight()) * 0.32f });
        gw.getWindow().draw(body);
        drawCenteredText("PRESS ENTER TO CONTINUE",
            static_cast<float>(gw.getHeight()) * 0.88f, 15, Color(0, 220, 255));
        gw.display();
    }
}
...
bool RealmOfEchoes::runGateChoice(int regionIndex, Region& region, const Encounter& enc)
{
    // Stop region music; play gate-choice music
    audio.Stop();
    audio.PlayMusic("assets/audio/GateChoice.ogg", 65.f);
    ...
    while (gw.isOpen())
    {
        while (auto event = gw.pollEvent())
        {
            ...
            if (auto* key = event->getIf<Event::KeyPressed>())
            {
                ...
                if (key->code == Keyboard::Key::Enter)
                {
                    ...
                    // Stop gate music before result display
                    audio.Stop();
                    ...
                    if (wrong)
                    {
                        gw.clear(); drawBackdrop();
                        drawCenteredText("THE TRAP GATE CLAIMS YOU!",
                            static_cast<float>(gw.getHeight()) * 0.4f, 24, Color(255, 30, 60));
                        gw.display();
                        sf::sleep(sf::seconds(2.f));
                        region.setEncounterIndex(0);
                        questLog.resetEncounterProgress(regionIndex);
                    ...
                    gw.clear(); drawBackdrop();
                    drawCenteredText("THE TRUE PATH OPENS.",
                        static_cast<float>(gw.getHeight()) * 0.45f, 22, Color(80, 255, 80));
                    gw.display();
                    sf::sleep(sf::seconds(2.f));
                    return true;
                }
        ...
        gw.clear();
        drawBackdrop();
        drawCenteredText("CHOOSE YOUR GATE", vh * 0.12f, 24, Color(0, 220, 255));
        drawCenteredText(enc.description, vh * 0.20f, 14, Color(230, 220, 255));
        ...
        Text leftLbl(font, "LEFT", 18);
        leftLbl.setFillColor(choice == 0 ? Color(255, 240, 80) : Color(200, 80, 255));
        FloatRect lb = leftLbl.getLocalBounds();
        leftLbl.setOrigin({ lb.size.x / 2.f, lb.size.y / 2.f });
        leftLbl.setPosition({ xL, lblY });
        gw.getWindow().draw(leftLbl);
        ...
        Text rightLbl(font, "RIGHT", 18);
        rightLbl.setFillColor(choice == 1 ? Color(255, 240, 80) : Color(200, 80, 255));
        FloatRect rb = rightLbl.getLocalBounds();
        rightLbl.setOrigin({ rb.size.x / 2.f, rb.size.y / 2.f });
        rightLbl.setPosition({ xR, lblY });
        gw.getWindow().draw(rightLbl);
...
// ─────────────────────────────────────────────────────────────────────────────
// PHANTOM SURVIVAL
// Wraith uses proper animation states:
//   PIdle    → walking/idle phase
//   PAttack  → wind-up attack frames
//   PMove    → closing-in frames
//   PSteal   → steal animation (plays at the final 0.2 s window)
//   PDie     → played after time-out if player still has XP
//   VBlock   → player blocks each hit
//   VDie     → player's die state if XP reaches 0 at final sting
// ─────────────────────────────────────────────────────────────────────────────
...
void RealmOfEchoes::runPhantomSurvival(int regionIndex, Region& region)
{
    ...
    // Stop whatever music is playing; start phantom music
    audio.Stop();
    audio.PlayMusic("assets/audio/Phantom_Wraith.ogg", 70.f);
    ...
    // ── Phantom frame indices ───────────────────────────────────────────────
    // We keep them named so it's obvious which sprite is which state.
    enum PhantomFrame { PIdle = 0, PAttack = 1, PMove = 2, PSteal = 3, PDie = 4, FRAME_COUNT };
    int currentFrame = PIdle;

    // Load all phantom frames
    vector<Texture> phantomTex(FRAME_COUNT);
    ...
    {
        if (AssetLoader::loadTextureWithFallback(phantomTex[i], { framePaths[i] }))
            phantomTex[i].setSmooth(true);
    }

    // Load Veilwalker combat sprites (Block / Die)
    Texture vBlockTex, vDieTex;
    optional<Sprite> vBlockSpr, vDieSpr;
    if (AssetLoader::loadTextureWithFallback(vBlockTex, { "assets/characters/VBlock.png" })) {
        vBlockTex.setSmooth(true);
        vBlockSpr.emplace(vBlockTex);
    }
    if (AssetLoader::loadTextureWithFallback(vDieTex, { "assets/characters/VDie.png" })) {
        vDieTex.setSmooth(true);
        vDieSpr.emplace(vDieTex);
    }

    // State machine
    bool finalStingTriggered = false;    // true once the last-attack window fires
    bool showPlayerBlock = false;    // show VBlock briefly after each hit
    Clock blockDisplayClock;
    const float blockDisplayTime = 0.35f;

    auto drawPhantom = [&](int frame)
        {
            if (phantomTex[frame].getSize().x == 0) return;
            Sprite spr(phantomTex[frame]);
            const float targetH = static_cast<float>(gw.getHeight()) * 0.38f;
            const float sc = targetH / static_cast<float>(phantomTex[frame].getSize().y);
            spr.setScale({ sc, sc });
            // Centre horizontally, a little above mid-screen
            spr.setPosition({ centerX() - spr.getGlobalBounds().size.x * 0.5f,
                              static_cast<float>(gw.getHeight()) * 0.28f });
            gw.getWindow().draw(spr);
        };

    auto drawVeilwalker = [&](bool isDying)
        {
            const auto& tex = isDying ? vDieTex : vBlockTex;
            if (tex.getSize().x == 0) return;
            Sprite spr(tex);
            const float targetH = static_cast<float>(gw.getHeight()) * 0.30f;
            const float sc = targetH / static_cast<float>(tex.getSize().y);
            spr.setScale({ sc, sc });
            spr.setPosition({ centerX() * 0.35f,
                              static_cast<float>(gw.getHeight()) * 0.45f });
            gw.getWindow().draw(spr);
        };
    ...
    while (gw.isOpen() && !done)
    {
        ...
        float remaining = kSurvivalSeconds - elapsed;
        ...
        // ── Normal phase: walking and attacking pattern ──────────────────────
        if (!finalStingTriggered)
        {
            // Alternate: approach (PMove) → wind-up (PAttack) → idle (PIdle)
            if (remaining > kSurvivalSeconds * 0.66f)
                currentFrame = PIdle;
            else if (remaining > kSurvivalSeconds * 0.33f)
                currentFrame = PMove;
            else
                currentFrame = PAttack;

            // Regular periodic hits
            if (hitClock.getElapsedTime().asSeconds() >= hitInterval)
            {
                hitClock.restart();
                player->takeRawDamage(1);
                ...
                blockDisplayClock.restart();
            }

            if (!player->isAlive()) { done = true; finalStingTriggered = true; }
            if (elapsed >= kSurvivalSeconds - kFinalStingThreshold)
                finalStingTriggered = true;
        }
        else
        {
            // ── Final-sting window (last 0.2 s) ────────────────────────────
            // Phantom shows PSteal; apply the last steal hit once
            currentFrame = PSteal;

            if (!done) // fire once
            {
                player->takeRawDamage(1);   // final sting
                audio.playOptional("hit");
                showPlayerBlock = true;
                blockDisplayClock.restart();
                done = true;
        ...
        if (showPlayerBlock && blockDisplayClock.getElapsedTime().asSeconds() > blockDisplayTime)
            showPlayerBlock = false;

        // Timer expired normally → survival done
        if (elapsed >= kSurvivalSeconds) done = true;

        // ── Draw ────────────────────────────────────────────────────────────
        gw.clear();
        drawBackdrop();
        drawCenteredText("SURVIVE THE WRAITH",
            static_cast<float>(gw.getHeight()) * 0.08f, 22, Color(255, 130, 0));

        // Phantom
        drawPhantom(currentFrame);

        // Veilwalker (block or die)
        bool playerDead = !player->isAlive();
        if (showPlayerBlock || playerDead)
            drawVeilwalker(playerDead);

        // Timer / HP
        ostringstream oss;
        oss.setf(ios::fixed); oss.precision(1);
        float disp = max(0.f, remaining);
        oss << "TIME LEFT: " << disp;
        drawCenteredText(oss.str(),
            static_cast<float>(gw.getHeight()) * 0.72f, 18, Color(0, 220, 255));
        drawCenteredText("HOLD YOUR GROUND. EACH STING TAKES 1 HP.",
            static_cast<float>(gw.getHeight()) * 0.80f, 13, Color(220, 210, 255));

        ostringstream hp;
        hp << "HP: " << player->getHp() << "/" << player->getMaxHp();
        drawCenteredText(hp.str(),
            static_cast<float>(gw.getHeight()) * 0.88f, 18, Color(80, 255, 120));
        gw.display();
    }

    // Stop phantom music before showing result
    audio.Stop();

    // ── Post-survival outcome ────────────────────────────────────────────────
    // Show final phantom animation: PSteal if player lost XP, PDie otherwise
    bool playerHasXP = (player->getXP() > 0);
    int resultFrame = playerHasXP ? PDie : PSteal;  // PDie = we survived, PSteal = XP stolen

    // Show result frame for a moment
    gw.clear();
    drawBackdrop();
    drawCenteredText("SURVIVE THE WRAITH",
        static_cast<float>(gw.getHeight()) * 0.08f, 22, Color(255, 130, 0));
    drawPhantom(resultFrame);
    if (!playerHasXP)
    {
        // VDie shown when XP is 0
        drawVeilwalker(true);
        drawCenteredText("YOUR SPIRIT ENERGY IS SPENT!",
            static_cast<float>(gw.getHeight()) * 0.72f, 18, Color(255, 60, 60));
    }
    else
    {
        drawCenteredText("YOU ENDURED THE WRAITH!",
            static_cast<float>(gw.getHeight()) * 0.72f, 18, Color(80, 255, 120));
    }
    gw.display();
    sf::sleep(sf::seconds(1.4f));
...
void RealmOfEchoes::showShrineScreen(int regionIndex)
{
    ...
    while (gw.isOpen() && !done)
    {
        ...
        drawBackdrop();
        drawCenteredText("ANCIENT SHRINE",
            static_cast<float>(gw.getHeight()) * 0.28f, 28, Color(210, 100, 255));
        drawCenteredText("YOU REST. HP AND SP FULLY RESTORED.",
            static_cast<float>(gw.getHeight()) * 0.42f, 16, Color(220, 210, 255));

        ostringstream stats;
        stats << "HP: " << player->getHp() << "/" << player->getMaxHp()
            << "   SP: " << player->getSpiritEnergy() << "/" << player->getMaxEnergy();
        drawCenteredText(stats.str(),
            static_cast<float>(gw.getHeight()) * 0.54f, 15, Color(80, 255, 120));
...
void RealmOfEchoes::showRegionCompleteScreen(int regionIndex)
{
    ...
    while (gw.isOpen() && !done)
    {
        ...
        gw.clear(); drawBackdrop();
        drawCenteredText("REGION CLEARED!",
            static_cast<float>(gw.getHeight()) * 0.28f, 26, Color(80, 255, 80));
        drawCenteredText(regionNames[regionIndex],
            static_cast<float>(gw.getHeight()) * 0.42f, 20, Color(0, 220, 255));
        drawCenteredText("SECURED.",
            static_cast<float>(gw.getHeight()) * 0.52f, 16, Color(220, 210, 255));
        drawCenteredText("PRESS ENTER TO RETURN TO THE VEILLANDS.",
            static_cast<float>(gw.getHeight()) * 0.88f, 14, Color(200, 80, 255));
        gw.display();
    }
}
...
void RealmOfEchoes::showEndingScreen()
{
    ...
    while (gw.isOpen() && !done)
    {
        ...
        gw.clear(); drawBackdrop();
        drawCenteredText("YOU HAVE WON!",
            static_cast<float>(gw.getHeight()) * 0.12f, 30, Color(255, 215, 0));
        drawCenteredText("THE SHADOW SOVEREIGN FALLS!",
            static_cast<float>(gw.getHeight()) * 0.24f, 22, Color(0, 220, 255));
        drawCenteredText("BALANCE RETURNS TO THE VEILLANDS.",
            static_cast<float>(gw.getHeight()) * 0.36f, 15, Color(220, 210, 255));
        drawCenteredText("VEILWALKER: " + upperText(player->getName()),
            static_cast<float>(gw.getHeight()) * 0.48f, 16, Color(80, 255, 120));
        drawCenteredText("THANK YOU FOR PLAYING!",
            static_cast<float>(gw.getHeight()) * 0.60f, 18, Color(200, 80, 255));
        drawCenteredText("PRESS ENTER TO RETURN TO ARCANE ARENA.",
            static_cast<float>(gw.getHeight()) * 0.88f, 14, Color(200, 80, 255));
        gw.display();
    }
...
void RealmOfEchoes::runRegion(int regionIndex)
{
    Region& region = regions[regionIndex];
    region.setEncounterIndex(questLog.getEncounterProgress(regionIndex));
    loadRegionBackground(regionIndex);   // plays region music
    gw.setInterface(region.getThemeColor(), "Realm of Echoes - " + region.getName());
    ...
    while (gw.isOpen() && !quitToHub)
    {
        ...
        if (enc.type == EncounterType::Shrine)
        {
            showShrineScreen(regionIndex);   // showShrineScreen restores region music on exit
            region.advanceEncounter();
            syncSave(regionIndex, region);
            continue;
        }
        ...
        if (enc.type == EncounterType::Trickster)
        {
            // Stop region music; play trickster music
            audio.Stop();
            audio.PlayMusic("assets/audio/Trickster.ogg", 65.f);
            ...
            if (AssetLoader::loadTextureWithFallback(tricksterTex, { "assets/characters/trickster.png" })) {
                ...
                const float sc = targetH / static_cast<float>(tricksterTex.getSize().y);
                tricksterSpr->setScale({ sc, sc });
                tricksterSpr->setPosition({ static_cast<float>(gw.getWidth()) * 0.72f,
                                            static_cast<float>(gw.getHeight()) * 0.32f });
            }
            ...
            while (gw.isOpen() && !done)
            {
                ...
                gw.clear(); drawBackdrop();
                drawCenteredText("TRICKSTER",
                    static_cast<float>(gw.getHeight()) * 0.14f, 26, Color(255, 130, 0));
                if (vNegSpr)   gw.getWindow().draw(*vNegSpr);
                if (tricksterSpr) gw.getWindow().draw(*tricksterSpr);
                body.setPosition({ 36.f, static_cast<float>(gw.getHeight()) * 0.28f });
                gw.getWindow().draw(body);
                drawCenteredText("PRESS ENTER",
                    static_cast<float>(gw.getHeight()) * 0.88f, 14, Color(0, 220, 255));
                gw.display();
            }

            // Stop trickster music; resume region music
            audio.Stop();
            audio.PlayMusic([&]() -> std::string {
                static const char* mp[4] = {
                    "assets/audio/region_forest.ogg",
                    "assets/audio/region_temple.ogg",
                    "assets/audio/region_mountain.ogg",
                    "assets/audio/region_veil.ogg",
                };
                return mp[regionIndex];
                }(), 50.f);
        ...
        if (enc.type == EncounterType::GateChoice)
        {
            if (runGateChoice(regionIndex, region, enc))
                region.advanceEncounter();
            // Wrong-gate reset is handled inside runGateChoice
            syncSave(regionIndex, region);
            continue;
        }
        ...
        if (enc.type == EncounterType::Battle)
        {
            // Stop region music; play battlefield music
            audio.Stop();
            audio.PlayMusic("assets/audio/BattleField.ogg", 65.f);
            ...
            // Restore region music after battle
            audio.Stop();
            audio.PlayMusic([&]() -> std::string {
                static const char* mp[4] = {
                    "assets/audio/region_forest.ogg",
            ...
            if (!player->getInventory().addItem(Item("Healing Potion", ItemType::HealingPotion, 30))) {
                // Inventory full — not a blocking error
            }
        ...
        if (enc.type == EncounterType::BossBattle)
        {
            ...
            while (gw.isOpen() && !quitToHub && bossAttemptsLeft > 0 && !bossDefeated)
            {
                ...
                // Stop region music; play boss intro music
                audio.Stop();
                audio.PlayMusic("assets/audio/boss_intro.ogg", 70.f);
                ...
                if (res == BattleResult::PlayerExited)
                {
                    audio.Stop();
                    return;
                }
                ...
                if (res == BattleResult::PlayerWon)
                {
                    ...
                    // Stop boss music, play defeat SFX
                    audio.Stop();
                    audio.playOptional("boss_defeat", 70.f);
                    ...
                    // Show region complete screen;
                    // Stop the boss_defeat sound before switching screens
                    showRegionCompleteScreen(regionIndex);
                    audio.StopSound("boss_defeat");   // ensure it doesn't bleed into next screen
                ...
                {
                    // Player died in boss fight — stop boss music
                    audio.Stop();
                    ...
                    if (bossAttemptsLeft > 0)
                    {
                        gw.clear(); drawBackdrop();
                        drawCenteredText("YOU FELL... " + to_string(bossAttemptsLeft) + " ATTEMPTS LEFT",
                            static_cast<float>(gw.getHeight()) * 0.45f, 22, Color(255, 120, 0));
                        gw.display();
                        sf::sleep(sf::seconds(2.f));
                    }
                    else
                    {
                        gw.clear(); drawBackdrop();
                        drawCenteredText("THE BOSS DEFEATED YOU!",
                            static_cast<float>(gw.getHeight()) * 0.4f, 24, Color(255, 60, 60));
                        drawCenteredText("YOU MUST CLEAR THE REGION AGAIN.",
                            static_cast<float>(gw.getHeight()) * 0.5f, 16, Color(80, 255, 120));
                        gw.display();
                        sf::sleep(sf::seconds(2.5f));
                        region.setEncounterIndex(0);
                        questLog.resetEncounterProgress(regionIndex);
...
void RealmOfEchoes::run()
{
    ...
    if (loadStatus != FileManager::LoadStatus::NotFound)
    {
        ...
        while (gw.isOpen() && !decided)
        {
            ...
            {
                drawCenteredText("SAVE FILE FOUND!",
                    static_cast<float>(gw.getHeight()) * 0.38f, 24, Color(0, 220, 255));
                drawCenteredText("L = LOAD    N = NEW GAME",
                    static_cast<float>(gw.getHeight()) * 0.48f, 16, Color(220, 210, 255));
            }
            else
            {
                drawCenteredText("SAVE FILE IS CORRUPT",
                    static_cast<float>(gw.getHeight()) * 0.36f, 24, Color(255, 120, 0));
                drawCenteredText("PRESS N FOR NEW GAME",
                    static_cast<float>(gw.getHeight()) * 0.48f, 16, Color(220, 210, 255));
            }
            gw.display();
